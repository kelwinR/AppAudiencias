...
public class CompController {
    ...
    public Comp buscar(String codigo)
    {
        Comp objComp = null;
        try {
            ...
            String wSQL = " SELECT * FROM comp WHERE id = ? ";
    ...
    public boolean verificaExistencia(Comp objeto)
    {
        try {
            //Conexao.abreConexao();
            Connection con = Conexao.getConnection();
            ...
            String wSQL = " SELECT id FROM comp WHERE nome = ?";
            stmt = con.prepareStatement(wSQL);
    ...
    public String incluir(Comp objeto)
    {
        try {
            Connection con = Conexao.getConnection();
            PreparedStatement stmt = null;
            ...
            if(verificaExistencia(objeto) == true){
                return "Competência já cadastrada";
            }else{
           
                String wSQL = " INSERT INTO comp VALUES(DEFAULT, ?)";
                stmt = con.prepareStatement(wSQL);
                stmt.setString(1, objeto.getNome());     
    ...
    public boolean alterar(Comp objeto){
        try {
            Connection con = Conexao.getConnection();
            PreparedStatement stmt = null;
            
            //VALIDAR SE O LOGIN EXISTE
                String wSQL = " UPDATE comp SET nome = ? WHERE id = ?";
                stmt = con.prepareStatement(wSQL);
                stmt.setString(1, objeto.getNome());               
    ...
    public boolean excluir(String codigo)
    {
        try {
            ...
            String wSQL = " DELETE FROM comp WHERE id = ? ";
            stmt = con.prepareStatement(wSQL);
            stmt.setInt(1, Integer.parseInt(codigo));
