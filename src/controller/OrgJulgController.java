 ...
 */
public class OrgJulgController {
    
    public OrgJulg buscar(String codigo)
    {
        OrgJulg objOrgJulg = null;
        try {
            Connection con = Conexao.getConnection();
            ResultSet rs = null;
            PreparedStatement stmt = null;
           
            String wSQL = " SELECT * FROM org_julg WHERE id = ? ";
    ...
    public boolean verificaExistencia(OrgJulg objeto)
    {
        try {
            //Conexao.abreConexao();
            Connection con = Conexao.getConnection();
            ...
            String wSQL = " SELECT id FROM org_julg WHERE nome = ?";
            stmt = con.prepareStatement(wSQL);
    ...
    public String incluir(OrgJulg objeto)
    {
        try {
            Connection con = Conexao.getConnection();
            PreparedStatement stmt = null;
            ...
            if(verificaExistencia(objeto) == true){
                return "Orgão Julgador já cadastrado";
            }else{
           
                String wSQL = " INSERT INTO org_julg VALUES(DEFAULT, ?)";
                stmt = con.prepareStatement(wSQL);
                stmt.setString(1, objeto.getNome());     
    ...
    public boolean alterar(OrgJulg objeto){
        try {
            Connection con = Conexao.getConnection();
            PreparedStatement stmt = null;
            
            //VALIDAR SE O LOGIN EXISTE
                String wSQL = " UPDATE org_julg SET nome = ? WHERE id = ?";
                stmt = con.prepareStatement(wSQL);
                stmt.setString(1, objeto.getNome());               
